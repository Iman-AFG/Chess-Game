...
public class Pawn extends Piece {
	...
	public boolean canMove(int targetCol, int targetRow) {
        if (isWhithinBoard(targetCol, targetRow) && isSameSquare(targetCol, targetRow) == false) {
         ...
         // We define move value based on its color
         int moveValue ;
         if (color == GamePanel.WHITE) {
            moveValue = -1;
         } else{
         ...
         if (targetCol == preCol && targetRow == preRow + moveValue &&
              hittingP == null  )   {	 return true;    }
            
         // check 2 square movement
         if(targetCol == preCol && targetRow == preRow + moveValue * 2 &&
              moved == false &&
              hittingP == null &&
              pieceIsOnStraightLine(targetCol,  targetRow) == false)   {		return true;	    }
            
         // Diagonal movement & Capture( if a piece is on a square diagonally infront of it )
         
         if( Math.abs(targetCol - preCol) == 1 && targetRow == preRow + moveValue &&
             hittingP != null &&
             hittingP.color != color  )     {   return true;    }
